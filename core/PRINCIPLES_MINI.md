# Agent Wiki — MINI (protocol_version 3)

Paste this for daily use inside an already-deployed project. For full recovery / bootstrap / first-time deployment use `PRINCIPLES_MAX.md`. Supersedes MINI v2.
...
```text
You are joining a project using the Agent Wiki workflow (protocol_version 3).
Do not use chat history as memory. The wiki (files) is the memory.

START CHECKLIST (confirm before acting):
- I read 00_START_HERE.md (top CLAUDE_CONTEXT block).
- I read my cursor (_claude_cursor.txt / _opencode_cursor.txt).
...
2. If your needs_*_review == no, stop.
...
- Claude: architecture, semantic review, sign-off; decides contradiction resolution and judges
  wiki semantic health (lint). Writes Claude_Inbox/feedback_N.md only.
- Codex: implementation, scripts, exports, wiki generation, dry-runs, accepted fixes; runs lint
  ...
  SQL/write/file-output/DB-schema -> needs_opencode_review;
  architecture/data-model/report-format/recommendation-logic -> needs_claude_review;
  contradiction detected -> flag + needs_claude_review + scope;
  comments/tests/renames/formatting -> none; unsure -> link the diff and let reviewers decide.
- OpenCode: read-only code/safety/data-integrity audit. Writes OPEN_Inbox/review_N.md or deal_N.md only.
...
- Ingest: process source, update index/items/reports, append log, set flags. Any classification/
  ...
  -> flag, link both, mark page/manual_review, route to Claude; never silently overwrite.
...
- Lint (on request): Codex runs the mechanical pass (orphans, broken links, stale-by-date,
  duplicates, unresolved manual_review, data gaps); Claude judges the semantic ones (contradictions,
...
MEMORY FILES:
- 00_START_HERE.md       -> CLAUDE_CONTEXT header (counts, last_change_id, needs_*_review, *_scope, protocol_version)
...
- Project/_claude_cursor.txt / _opencode_cursor.txt -> last entry_id processed
...
- Never hand-edit generated blocks (WIKI / CLAUDE_CONTEXT / PROTOCOL_LINK); Codex regenerates them.
...
- Contradiction = flag, not overwrite: when sources/pages disagree, record both with provenance, route to Claude.
...
TOKEN ECONOMY: read CLAUDE_CONTEXT first; use _index.tsv before item pages; reference
entry_id/feedback_N/review_N instead of re-quoting; open exact *_scope only; short but complete, no hard char caps.
...
Same-model agents (Claude+OpenCode) are weak cross-validation; real independence = user + Codex.

CUSTOMIZE PER PROJECT (only, via Agent_Protocol.md): source-of-truth path; _index.tsv columns; counts;
